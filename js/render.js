...
export function spawnFloater(x, y, text, color, opts = {}) {
  fx.floaters.push({
    ...
    text,
    color,
    life: opts.small ? 0.7 : 1.1,
    max: opts.small ? 0.7 : 1.1,
    vy: -1.2,
...
export function handleFxEvent(ev) {
  switch (ev.t) {
    ...
    case 'fx':
      switch (ev.k) {
        case 'slash':
          spawnParticles(ev.x, ev.y, ev.c || '#ffffff', 6, {
            angle: ev.a,
            spread: 1.1,
            speed: 4,
            life: 0.25,
        ...
        case 'wave':
          ...
          spawnBeam(
            ev.x,
            ev.y,
            ev.x + Math.cos(ev.a) * ev.r,
            ev.y + Math.sin(ev.a) * ev.r,
            ev.c,
            0.22
          );
          break;
        ...
        case 'cast':
          ...
          break;
        case 'death':
          spawnParticles(ev.x, ev.y, ev.c, ev.boss ? 60 : 16, {
            speed: ev.boss ? 9 : 5,
            life: ev.boss ? 1.2 : 0.7,
            size: ev.boss ? 5 : 3,
          });
          if (ev.boss) {
            spawnRing(ev.x, ev.y, 5, '#ff9c2f', { life: 1.0, width: 8 });
            cam.shake = 22;
          }
          break;
        case 'heal':
...
export function drawWorld(ctx, canvas, view, now) {
  ...
  const halfW = canvas.width / 2 / cam.zoom,
    halfH = canvas.height / 2 / cam.zoom;
  const corners = [
    [cam.x - halfW, cam.y - halfH],
    [cam.x + halfW, cam.y - halfH],
    [cam.x - halfW, cam.y + halfH],
    [cam.x + halfW, cam.y + halfH],
  ...
  const mark = P
    ? k => {
        ...
        if (P.last != null) P.t[P.k] = (P.t[P.k] || 0) + n - P.last;
        P.k = k;
        P.last = n;
      }
    : () => {};
  ...
  for (let y = minY; y <= maxY; y++) {
    for (let x = minX; x <= maxX; x++) {
      ...
      if (t === T.WALL || t === T.BRAZIER) {
        walls.push({ x, y, t, px: p.x, py: p.y, depth: x + y });
        continue;
      }
      ...
      if (t === T.LAVA) {
        ...
        for (let i = 0; i < 3; i++) {
          const gx = p.x - 8 + i * 8 + n * 5,
            gy = p.y + 2;
          grassPath.moveTo(gx, gy);
          grassPath.lineTo(gx + (n - 0.5) * 4, gy - 6 - n * 3);
        }
      } else if (t === T.RUBBLE) {
  ...
  if (waterPulse) {
    ctx.fillStyle = `rgba(90,160,220,${waterPulse})`;
    ctx.fill(waterPath);
  }
  ...
  if (players.length > 1 && local) {
    ...
    for (const o of perto && pertoD >= EMBER_LINK_MIN ? [perto] : []) {
      const d = pertoD;
      const a = project(local.x, local.y),
        b = project(o.x, o.y);
      const steps = 22;
      ctx.save();
      ctx.globalCompositeOperation = 'lighter';
...
function flushWalls(acc, ctx) {
  ...
  if (acc.braziers) {
    ...
    for (const b of acc.braziers) {
      ...
      ctx.fillStyle = g2;
      ctx.beginPath();
      ctx.arc(b.px, b.py - WALL_H - 8, r, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
    acc.braziers = null;
...
function drawItem(ctx, it, now) {
  ...
  ctx.fillStyle = g;
  ctx.beginPath();
  ctx.arc(p.x, p.y - 8 + bob, 22, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
...
export function drawGlyph(ctx, glyph, x, y, scale, color) {
  ...
  switch (glyph) {
    ...
    case 'bow':
      ...
      ctx.lineWidth = 2.5;
      ctx.beginPath();
      ctx.arc(0, 0, 9, -Math.PI / 2.2, Math.PI / 2.2);
      ctx.stroke();
    ...
    case 'amulet':
      ...
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.arc(0, -4, 7, 0.2 * Math.PI, 0.8 * Math.PI, true);
      ctx.stroke();
      ctx.fillStyle = color;
      ctx.beginPath();
      ctx.moveTo(0, 1);
...
function drawCreature(ctx, px, py, shape, color, size, opts) {
  ...
  const paint = h => {
    ctx.fillStyle = grad || bodyGradient(ctx, 0, 0, fill, h);
  };
  ...
  switch (shape) {
    case 'bones':
      ...
      for (let i = 0; i < 3; i++) {
        ctx.beginPath();
        ctx.moveTo(-4, -4 - i * 3.5);
        ctx.lineTo(4, -4 - i * 3.5);
        ctx.stroke();
      }
      ...
      ctx.beginPath();
      ctx.arc(-2.4, -21, 1.4, 0, Math.PI * 2);
      ctx.arc(2.4, -21, 1.4, 0, Math.PI * 2);
      ctx.fill();
      break;
    ...
    case 'beast':
      ...
      ctx.beginPath();
      ctx.moveTo(-10, -10);
      ctx.quadraticCurveTo(-17, -14, -14, -4);
      ctx.lineWidth = 2.4 / s;
      ctx.strokeStyle = grad || darken(fill, 0.2);
      ctx.stroke();
      ctx.fillStyle = '#ffcf6b';
      ctx.beginPath();
      ctx.arc(11, -14, 1.3, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = 'rgba(0,0,0,0.5)';
      ctx.lineWidth = 1.6 / s;
    ...
    case 'robed':
      ...
      ctx.quadraticCurveTo(6, -20, 10, 1);
      ctx.closePath();
      ctx.fill();
      ctx.stroke();
      ctx.fillStyle = darken(grad || fill, 0.55);
      ctx.beginPath();
      ctx.ellipse(0, -22, 6, 6.5, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
      ...
      ctx.arc(2, -22, 1.2, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = darken(grad || fill, 0.2);
      ctx.lineWidth = 2 / s;
      ctx.beginPath();
      ctx.moveTo(9, -26);
      ctx.lineTo(11, -2);
    ...
    case 'brute':
      ...
      ctx.beginPath();
      ctx.arc(-2.6, -25, 1.6, 0, Math.PI * 2);
      ctx.arc(2.6, -25, 1.6, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = grad || darken(fill, 0.15);
      ctx.beginPath();
      ctx.ellipse(-13, -12, 3.5, 7, 0.3, 0, Math.PI * 2);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(13, -12, 3.5, 7, -0.3, 0, Math.PI * 2);
      ctx.fill();
    ...
    case 'wisp': {
      ...
      ctx.beginPath();
      ctx.moveTo(-8, 2 + t);
      ctx.quadraticCurveTo(-9, -18, 0, -22);
      ctx.quadraticCurveTo(9, -18, 8, 2 + t);
      ctx.quadraticCurveTo(4, -2, 0, 2);
      ctx.quadraticCurveTo(-4, -2, -8, 2 + t);
      ctx.closePath();
      ctx.fill();
      ctx.stroke();
      ctx.fillStyle = '#0b0910';
      ctx.beginPath();
      ctx.ellipse(-2.6, -19, 1.5, 2.2, 0, 0, Math.PI * 2);
      ctx.ellipse(2.6, -19, 1.5, 2.2, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.globalAlpha = 1;
      break;
    ...
    case 'dragon':
      ...
      ctx.beginPath();
      ctx.moveTo(-4, -16);
      ctx.quadraticCurveTo(-22, -30, -14, -8);
      ctx.closePath();
      ctx.fill();
      ctx.stroke();
      ctx.beginPath();
      ...
      paint(24);
      ctx.beginPath();
      ctx.ellipse(0, -11, 10, 8, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
      ctx.beginPath();
      ctx.ellipse(9, -20, 6.5, 4.5, -0.4, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
      ctx.fillStyle = '#ffd84d';
      ctx.beginPath();
      ctx.arc(11, -21, 1.5, 0, Math.PI * 2);
      ctx.fill();
      break;
...
function drawMonster(ctx, m, now, view) {
  ...
  if (m.status?.freeze > 0) {
    ...
    ctx.beginPath();
    ctx.ellipse(p.x, p.y - 14, 16 * msz, 22 * msz, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }

  drawCreature(ctx, p.x, p.y, m.shape, m.color, msz, {
    facing,
    bob,
    flash: m.hitFlash > 0 ? 1 : 0,
  });

  const elite = eliteLabel(m);
  if (elite) {
    ctx.save();
    ...
    ctx.lineWidth = 3;
    ctx.strokeText(elite, p.x, p.y - 40 * msz);
    ctx.fillText(elite, p.x, p.y - 40 * msz);
    ctx.restore();
  }

  if (m.status?.burn > 0) {
    for (let i = 0; i < 2; i++) {
      const fx2 = p.x + (Math.random() - 0.5) * 14,
        fy = p.y - 10 - Math.random() * 18;
  ...
  if (m.isBoss) {
    ...
    ctx.fillStyle = g;
    ctx.beginPath();
    ctx.arc(p.x, p.y - 10, 60, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
  if (m.windup > 0) {
    ...
    ctx.ellipse(
      ...
      0,
      0,
      Math.PI * 2 * telegraphProgress(m.windup, telegraphTotal(m))
    );
    ctx.stroke();
  } else {
...
function drawPlayerStatus(ctx, p, pos, now) {
  ...
  if (st.freeze > 0) {
    ...
    ctx.beginPath();
    ctx.ellipse(pos.x, pos.y - 14, 15, 21, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
  if (st.stun > 0) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      ctx.beginPath();
      ctx.arc(pos.x + Math.cos(a) * 11, pos.y - 30 + Math.sin(a) * 4, 2, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  if (st.burn > 0) {
    for (let i = 0; i < 2; i++) {
      const fx2 = pos.x + (Math.random() - 0.5) * 13,
        fy = pos.y - 10 - Math.random() * 17;
      ctx.fillStyle = `rgba(255,${(120 + Math.random() * 90) | 0},40,0.7)`;
      ctx.fillRect(fx2, fy, 2.5, 2.5);
    }
  }
...
function drawPlayer(ctx, p, now, view, lights) {
  ...
  ctx.beginPath();
  ctx.moveTo(-9, 0);
  ctx.quadraticCurveTo(-11, -18, -3, -24);
  ctx.lineTo(3, -24);
  ctx.quadraticCurveTo(11, -18, 9, 0);
  ctx.closePath();
  ctx.fill();
  ...
  ctx.beginPath();
  ctx.arc(0, -23, 5.5, 0, Math.PI * 2);
  ctx.fill();
  ctx.stroke();
  ctx.fillStyle = darken(V.color, 0.2);
  ctx.beginPath();
  ctx.arc(0, -25, 5.6, Math.PI, Math.PI * 2);
  ctx.fill();
  ...
  if (V.id === 'knight') {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(0, -6, 9, -Math.PI / 2.2, Math.PI / 2.2);
    ctx.stroke();
  } else {
    ctx.fillStyle = '#4a3c2c';
  ...
  if (p.casting > 0) {
    ...
    ctx.fillStyle = cg;
    ctx.beginPath();
    ctx.arc(pos.x, pos.y - 12, 34, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
...
function drawProjectile(ctx, pr, lights) {
  ...
  ctx.fillStyle = g;
  ctx.beginPath();
  ctx.arc(p.x, p.y - 14, size * 3.4, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = '#fff';
  ctx.beginPath();
  ctx.arc(p.x, p.y - 14, size * 0.5, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
...
function drawLighting(ctx, canvas, lights, view, now) {
  ...
  for (const L of lights) {
    ...
    if (s.x + r < 0 || s.x - r > lightCanvas.width || sy + r < 0 || sy - r > lightCanvas.height)
      continue;
...
export function drawMinimap(mm, view, now = 0) {
  ...
  const ox = local.x - span / 2,
    oy = local.y - span / 2;

  for (let y = 0; y < span; y++) {
    for (let x = 0; x < span; x++) {
      const tx = Math.floor(ox + x),
        ty = Math.floor(oy + y);
      if (tx < 0 || ty < 0 || tx >= map.w || ty >= map.h) continue;
      const t = map.tiles[ty * map.w + tx];
      if (t === T.VOID) continue;
      ctx.fillStyle =
        t === T.WALL || t === T.BRAZIER
          ? '#241f2c'
          : t === T.LAVA
            ? '#5a1e0c'
            : t === T.WATER
  ...
  if (view.portalOpen) {
    const x = (map.portal.x - ox) * scale,
      y = (map.portal.y - oy) * scale;
  ...
  for (const p of players) {
    ...
    ctx.beginPath();
    ctx.arc(x, y, noTrilho ? 2.5 : 2, 0, Math.PI * 2);
    ctx.fill();
    ctx.globalAlpha = 1;
  }
