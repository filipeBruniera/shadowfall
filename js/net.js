...
export class Net {
  ...
  host(code) {
    return new Promise((resolve, reject) => {
      if (typeof Peer === 'undefined') {
        reject(new Error('PeerJS não carregou. Verifique sua conexão.'));
        return;
      }
      ...
      this.peer.on('error', err => {
        ...
        if (err.type === 'unavailable-id')
          reject(new Error('Esse código já está em uso. Tente outro.'));
  ...
  join(code) {
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(
        () => reject(new Error('Não achei essa sala. Confira o código.')),
        20000
      );
      ...
      this.peer.on('error', err => {
        ...
        if (err.type === 'peer-unavailable')
          reject(new Error('Sala não encontrada. Confira o código.'));
        else if (!this.connected) reject(err);
        else this.emit('error', err);
      });
    });
  ...
  refuse(peerId, reason) {
    ...
    if (!c) return;
    try {
      if (c.open) c.send({ t: 'refused', reason });
    } catch (e) {
      /* já caiu */
    }
    setTimeout(() => {
  ...
  const snap = {
    ...
    P: players.map(p => {
      ...
      return {
        ...
        c: r2(p.anim.cast),
        hu: r2(p.anim.hurt),
        mv: p.anim.moving ? 1 : 0,
        b: p.buffs.length ? 1 : 0,
        cd: [p.skillCd.Q, p.skillCd.W, p.skillCd.E, p.skillCd.R].map(r2),
        la: p.lastAct,
        // A versão já existia no estado autoritativo e sobe em toda mutação de
        // inventário. Junto de `ti`, liga o snapshot à mensagem `inv` do dono.
        iv: p.invVer,
        k: p.kills,
        op: p.onPortal ? 1 : 0,
        ...(ps ? { s: ps } : null),
    ...
    R: G.projectiles
      .filter(pr => near(pr.x, pr.y))
      .map(pr => ({ i: pr.id, x: r2(pr.x), y: r2(pr.y), e: pr.elem, b: pr.big ? 1 : 0 })),
    Z: G.zones
      .filter(z => near(z.x, z.y))
      .map(z => ({ i: z.id, x: r2(z.x), y: r2(z.y), r: r2(z.r), c: z.color })),
  };
  ...
  for (const m of G.monsters) {
    ...
    if (viewer && aoi) {
      ...
      for (const p of players) {
        if (Math.abs(p.x - m.x) < AOI_RADIUS && Math.abs(p.y - m.y) < AOI_RADIUS) {
          visible = true;
          break;
        }
      }
    ...
    if (m.isBoss) {
      e.b = 1;
      e.n = m.name;
      if (m.hardcore) e.hc = 1;
    }
    if (m.elite) e.el = m.elite;
...
export function applySnapshot(view, snap) {
  ...
  for (const sp of snap.P) {
    ...
    p.moving = !!sp.mv;
    p.buffed = !!sp.b;
    p.skillCd = { Q: sp.cd[0], W: sp.cd[1], E: sp.cd[2], R: sp.cd[3] };
    p.lastAct = sp.la;
    p.kills = sp.k;
    p.onPortal = !!sp.op;
  ...
  for (const p of view.playerMap.values()) {
    if (p.id === localId && predicted) {
      ...
      if (d > 2.2) {
        ...
        predicted.x += (p.rx - predicted.x) * Math.min(1, dt * 5);
        predicted.y += (p.ry - predicted.y) * Math.min(1, dt * 5);
      }
      p.x = predicted.x;
      p.y = predicted.y;
    } else {
...
export function isCriticalEvent(ev) {
  return (
    CRITICAL.has(ev.t) ||
    (ev.t === 'fx' && (ev.k === 'playerDeath' || ev.k === 'revive')) ||
    !!ev.boss
  );
}
