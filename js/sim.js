...
import { applyElite, eliteAttackCooldown, rollElite } from './elite.js';
import { recordBestiaryDefeat } from './bestiary.js';
import {
  TICK,
  INV_SIZE,
  PICKUP_RADIUS,
...
export function addPlayer(G, { id, name, voc }) {
  ...
  const p = {
    ...
    invVer: 1,
    kills: 0,
    // Só as derrotas ficam entre runs. A guarda da morte fica na entidade da
    // run, pois ids de monstro reiniciam a cada novo mapa.
    bestiary: { kills: {} },
    // A projeção diária também é permanente, mas a lista e a deduplicação da
    // run ficam fora do simulador em `contracts.js`/`main.js`.
...
export function stats(p) {
  ...
  for (const slot of EQUIP_SLOTS) {
    ...
    s.cooldown += it.cooldown || 0;
    s.elemental += it.elemental || 0;
    s.statusPower += it.statusPower || 0;
    s.area += it.area || 0;
  }
...
export function step(G, dt) {
  ...
  // A lista é a mesma para todos os monstros deste tique. Recriá-la dentro de
  // updateMonster fazia uma alocação por inimigo (centenas no andar cheio),
  // que aparecia como pausas de GC no orçamento individual de `step()`.
  ...
  for (const p of players) updatePlayer(G, p, dt, players);
  updatePickup(G);
  updatePortal(G, dt);
  for (const m of G.monsters) updateMonster(G, m, dt, players);
  updateProjectiles(G, dt);
  updateZones(G, dt);
  updateBossEngagement(G);
...
function updatePlayer(G, p, dt, players) {
  const st = stats(p);
  ...
  if (p.dead) {
    ...
    for (const o of players) {
      ...
      if (dist(o, p) < REVIVE_RADIUS) {
        helpers++;
        if (!reviver) reviver = o;
      }
    }
...
function updatePickup(G) {
  ...
  for (let i = G.items.length - 1; i >= 0; i--) {
    ...
    for (const p of players) {
      ...
      if (
        d < best - 1e-9 ||
        (Math.abs(d - best) <= 1e-9 && winner && String(p.id) < String(winner.id))
      ) {
        best = d;
        winner = p;
      }
...
function handleAct(G, p, a, st) {
  switch (a.k) {
    case 'cast':
      castSkill(G, p, a.slot, a.ax, a.ay, st);
      break;
    case 'pot':
      usePotion(G, p, a.slot, st);
      break;
    case 'use':
      useInvSlot(G, p, a.slot);
      break;
    case 'drop':
      dropInvSlot(G, p, a.slot);
      break;
    case 'unequip':
      unequip(G, p, a.slot);
      break;
...
function castSkill(G, p, slotKey, ax, ay, st) {
  ...
  if (p.mp < skill.mana) {
    log(G, 'Mana insuficiente.', 'warn');
    return;
  }
  if (p.status.stun > 0 || p.status.freeze > 0) return;

  p.mp -= skill.mana;
  p.skillCd[slotKey] = skill.cd * Math.max(0.4, 1 - st.cooldown);
  p.anim.cast = 0.3;

  const aimA = Math.atan2(ay - p.y, ax - p.x);
  p.dir = aimA;
  const power = st.atk * 0.8 + st.ml * 2.0 + p.level * 2;
  // Atributo elemental não pode aumentar a magia física: o afixo representa
  // afinidade com um elemento, não um multiplicador genérico de habilidade.
  const damagePower = power * (1 + (skill.elem === E.PHYS ? 0 : st.elemental));
  const areaScale = 1 + st.area;
  ...
  switch (skill.type) {
    case 'bolt':
      spawnProjectile(G, {
        ...
        ty: ay,
        speed: skill.speed,
        dmg: damagePower * skill.mult,
        elem: skill.elem,
        ownerId: p.id,
        owner: 'player',
        range: skill.range,
        burn: scaledStatus(skill.burn, st),
        big: true,
      });
      break;
    ...
    case 'wave': {
      ...
      for (const m of G.monsters) {
        ...
        if (along > -0.4 && along < skill.range && side < skill.width * areaScale + m.size * 0.3)
          hits.push(m);
      }
      for (const m of hits) applySkillHit(G, p, m, damagePower * skill.mult, skill, st);
      pushEvent(G, {
        t: 'fx',
        k: 'wave',
        x: p.x,
        ...
        a: aimA,
        r: skill.range,
        w: skill.width * areaScale,
        c: ELEM_COLOR[skill.elem],
      });
      break;
    }
    ...
    case 'nova': {
      for (const m of G.monsters) {
        if (m.hp <= 0) continue;
        if (dist(m, p) <= skill.radius * areaScale + m.size * 0.4)
          applySkillHit(G, p, m, damagePower * skill.mult, skill, st);
      }
      pushEvent(G, {
        t: 'fx',
        k: 'nova',
    ...
    case 'ground': {
      G.zones.push({
        ...
        elem: skill.elem,
        dmg: damagePower * skill.mult,
        time: skill.time * (1 + st.statusPower),
        tick: skill.tick,
        tickTimer: 0,
        ownerId: p.id,
        poison: scaledStatus(skill.poison, st),
        color: ELEM_COLOR[skill.elem],
      });
      pushEvent(G, {
        t: 'fx',
        k: 'ground',
    ...
    case 'heal': {
      ...
      if (skill.ally) {
        let worst = null,
          worstPct = skill.allyFirst ? 1.01 : p.hp / st.maxHp;
    ...
    case 'buff': {
      ...
      if (skill.taunt) {
        for (const m of G.monsters) {
          if (m.hp > 0 && dist(m, p) <= skill.radius * areaScale) {
            m.tauntedBy = p.id;
            m.tauntTime = TAUNT_TIME;
            m.aggro = p.id;
          }
        }
      }
...
function applySkillHit(G, p, m, dmg, skill, st) {
  ...
  if (m.hp <= 0) return;
  const statusScale = 1 + st.statusPower;
  if (skill.stun) m.status.stun = Math.max(m.status.stun, skill.stun * statusScale);
  if (skill.freeze) m.status.freeze = Math.max(m.status.freeze, skill.freeze * statusScale);
  if (skill.slow) {
    m.status.slow = Math.max(m.status.slow, skill.slow.time * statusScale);
    m.status.slowMult = skill.slow.mult;
  }
  if (skill.burn) {
    m.status.burn = Math.max(m.status.burn, skill.burn.time * statusScale);
    m.status.burnDps = (skill.burn.dps + st.ml) * statusScale;
  }
  if (skill.poison) {
    m.status.poison = Math.max(m.status.poison, skill.poison.time * statusScale);
    m.status.poisonDps = (skill.poison.dps + st.ml * 0.5) * statusScale;
  }
}

function scaledStatus(effect, st) {
  if (!effect) return null;
  const scale = 1 + st.statusPower;
  return {
    ...effect,
    ...(effect.time ? { time: effect.time * scale } : null),
    ...(effect.dps ? { dps: effect.dps * scale } : null),
  };
}
...
export function hitMonster(G, m, raw, elem, source, opts = {}) {
  ...
  pushEvent(G, {
    ...
    y: m.y,
    v: String(dmg),
    c: opts.crit ? '#ffd84d' : ELEM_COLOR[elem],
    crit: !!opts.crit,
    big: mult > 1.2,
  });
...
function killMonster(G, m, source) {
  // Dano contínuo, projéteis atrasados ou um replay local podem voltar a
  // alcançar o cadáver. A transição viva→morta é única e tudo que concede
  // progresso precisa ficar depois desta guarda.
  if (!m || m.deathHandled) return false;
  m.deathHandled = true;
  m.hp = 0;
  m.deathFade = 0.6;
  if (m.isBoss) {
    const index = G.engagedBosses.indexOf(m);
    if (index >= 0) G.engagedBosses.splice(index, 1);
    m.engaged = false;
  }
  ...
  cancelWindup(m);
  // A apresentação continua consumindo `fx/death`; id e typeId tornam a
  // mesma confirmação autoritativa observável por bestiário e contratos sem
  // criar um segundo evento de morte que poderia divergir na rede.
  pushEvent(G, {
    t: 'fx',
    k: 'death',
    id: m.id,
  ...
  const killer = source?.id ? G.players[source.id] : null;
  if (killer) {
    killer.kills++;
    killer.bestiary = {
      kills: recordBestiaryDefeat(killer.bestiary?.kills, m.typeId),
    };
  }
...
// Uma troca de dano abre a luta; o mesmo campo também fecha o intervalo no
// tique, para que nenhum dos dois eventos possa se repetir sem o outro.
function markBossDamage(G, m) {
  if (!m || !m.isBoss) return;
  m.lastDamageTime = G.time;
  if (m.engaged) return;
  m.engaged = true;
  G.engagedBosses.push(m);
  pushEvent(G, {
    t: 'bossEngage',
    id: m.id,
    typeId: m.typeId,
    floor: G.floor,
    hardcore: m.hardcore ? 1 : 0,
    boss: 1,
  });
}

// Só chefes engajados entram nesta lista, mantendo o custo nulo fora de luta e
// evitando varrer a população inteira a cada tique para encerrar uma faixa.
function updateBossEngagement(G) {
  for (let i = G.engagedBosses.length - 1; i >= 0; i--) {
    const m = G.engagedBosses[i];
    if (!m || m.hp <= 0) {
      G.engagedBosses.splice(i, 1);
      continue;
    }
    if (G.time - m.lastDamageTime < BOSS_DISENGAGE_TIME) continue;
    m.engaged = false;
    G.engagedBosses.splice(i, 1);
    pushEvent(G, {
      t: 'bossDisengage',
...
export function damagePlayer(G, p, raw, elem, opts = {}) {
  ...
  const effective = Math.min(p.hp, dmg);
  p.hp -= dmg;
  p.anim.hurt = 0.2;
  if (!opts.silent) {
    pushEvent(G, {
...
// MONSTROS
// ============================================================
function updateMonster(G, m, dt, players) {
  if (m.hp <= 0) {
  ...
  if (m.tauntTime > 0) {
    m.tauntTime -= dt;
    if (m.tauntTime <= 0) m.tauntedBy = null;
  }
  ...
  // Alvo
  let target = null;
  if (m.tauntedBy && G.players[m.tauntedBy] && !G.players[m.tauntedBy].dead)
    target = G.players[m.tauntedBy];
  ...
  const attackRange =
    m.ai === 'ranged' || m.ai === 'caster' || m.ai === 'boss' ? 7.5 : 0.9 + m.size * 0.4;
  ...
  if (d <= attackRange && m.cd <= 0 && m.windup <= 0) {
    m.cd =
      (m.ai === 'boss' ? 1.5 : m.ai === 'caster' || m.ai === 'ranged' ? 2.2 : 1.4) *
      eliteAttackCooldown(m);
  ...
  switch (sp.kind) {
    case 'nova': {
      for (const p of Object.values(G.players)) {
        ...
        if (dist(p, m) < sp.r) {
          markBossDamage(G, m);
          damagePlayer(G, p, m.atk * sp.mult, m.elem);
        }
      }
    ...
    case 'burst': {
      for (let i = 0; i < sp.count; i++) {
        ...
        spawnProjectile(G, {
          x: m.x,
          y: m.y,
          tx: m.x + Math.cos(a) * sp.range,
          ty: m.y + Math.sin(a) * sp.range,
          speed: sp.speed,
          dmg: m.atk * sp.mult,
          elem: m.elem,
          ownerId: m.id,
...
function resolveMonsterAttack(G, m, target, attackRange) {
  ...
  if (m.ai === 'ranged' || m.ai === 'caster' || m.ai === 'boss') {
    ...
    markBossDamage(G, m);
    const effective = damagePlayer(G, p, m.atk, m.elem);
    pushEvent(G, { t: 'fx', k: 'slash', x: p.x, y: p.y, a: m.dir, c: m.color });
    if (m.lifesteal && effective > 0) m.hp = Math.min(m.maxHp, m.hp + effective * m.lifesteal);
    if (m.poison) {
      p.status.poison = Math.max(p.status.poison, m.poison.time);
      p.status.poisonDps = m.poison.dps;
    }
    ...
    for (let dy = -1; dy <= 1; dy++) {
      for (let dx = -1; dx <= 1; dx++) {
        ...
        if (
          dx &&
          dy &&
          (collides(map, nx + 0.5, my + 0.5, 0.3) || collides(map, mx + 0.5, ny + 0.5, 0.3))
        )
          continue;
...
function tickStatus(G, e, dt, isPlayer) {
  ...
  if (s.burn > 0) {
    ...
    if (isPlayer) damagePlayer(G, e, dmg, E.FIRE, { silent: true });
      ...
      e.hp -= dmg;
      if (e.hp <= 0) killMonster(G, e, null);
    }
  }
  if (s.poison > 0) {
    s.poison -= dt;
...
function spawnProjectile(G, o) {
  ...
  G.projectiles.push({
    ...
    owner: o.owner,
    ownerId: o.ownerId,
    life: (o.range || 8) / o.speed,
    homing: o.homing || 0,
    burn: o.burn || null,
    big: !!o.big,
    a,
    bossElem: o.bossElem == null ? null : o.bossElem,
    bossRef: o.bossRef || null,
  });
}
  ...
  for (let i = G.projectiles.length - 1; i >= 0; i--) {
    ...
    if (pr.homing) {
      ...
      if (t) {
        ...
        pr.vx = Math.cos(na) * speed;
        pr.vy = Math.sin(na) * speed;
        pr.a = na;
      }
    }
    pr.x += pr.vx * dt;
    ...
    if (pr.owner === 'player') {
      ...
      for (const m of G.monsters) {
        ...
        if (pr.burn && m.hp > 0) {
          m.status.burn = Math.max(m.status.burn, pr.burn.time);
          m.status.burnDps = pr.burn.dps;
        }
        pushEvent(G, { t: 'fx', k: 'impact', x: pr.x, y: pr.y, c: ELEM_COLOR[pr.elem] });
        G.projectiles.splice(i, 1);
        break;
      ...
      for (const p of Object.values(G.players)) {
        ...
        if (pr.bossRef) markBossDamage(G, pr.bossRef);
    ...
    if (z.tickTimer <= 0) {
      ...
      if (z.owner === 'monster') {
        for (const p of Object.values(G.players)) {
          ...
          if (z.bossRef) markBossDamage(G, z.bossRef);
...
function dropLoot(G, m, guaranteed) {
  ...
  G.items.push({
    id: G.nextId++,
    kind: 'gold',
    x: m.x + rng.range(-0.3, 0.3),
    y: m.y + rng.range(-0.3, 0.3),
    amount: Math.floor((6 + m.level * 4) * rng.range(0.7, 1.5)),
    glyph: 'gold',
    rarity: 'common',
    name: 'Ouro',
  });
  if (rng.chance(0.3)) {
    ...
    G.items.push({
      ...
      kind: 'potion',
      potion: isHp ? 'hp' : 'mp',
      x: m.x + rng.range(-0.5, 0.5),
      y: m.y + rng.range(-0.5, 0.5),
      glyph: isHp ? 'potionRed' : 'potionBlue',
      rarity: 'common',
...
export function rollItem(G, ilvl, forceGood = false) {
  ...
  const weights = RARITY_ORDER.map(r => [
    r,
    forceGood ? (r === 'common' ? 5 : RARITY[r].weight * 3) : RARITY[r].weight,
  ]);
...
function equipFromInv(G, p, slot) {
  ...
  if (!canUse(p, it)) {
    log(G, `${VOCATIONS[p.voc].name} não usa ${it.name}.`, 'warn');
    return;
  }
...
// Vender só as duas raridades anunciadas pela interface. Épico também é uma
// recompensa de build e nunca pode desaparecer num atalho de limpeza.
export function sellJunk(G, p) {
  let total = 0;
  for (let i = 0; i < p.inv.length; i++) {
    const it = p.inv[i];
    if (!it || it.kind !== 'equip') continue;
    if (it.rarity !== 'common' && it.rarity !== 'rare') continue;
...
export function moveEntity(map, e, dx, dy, r = 0.32) {
  if (dx) {
    const nx = e.x + dx;
    if (!collides(map, nx, e.y, r)) e.x = nx;
  }
  if (dy) {
    const ny = e.y + dy;
    if (!collides(map, e.x, ny, r)) e.y = ny;
  }
}
...
function pushPending(G, ev) {
  (G.pendingEvents || (G.pendingEvents = [])).push(ev);
}
function logPending(G, m, c) {
  pushPending(G, { t: 'log', m, c });
}
