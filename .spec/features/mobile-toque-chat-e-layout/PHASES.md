...
> **Status: implementado e coberto nos harnesses mobile.** As caixas abaixo preservam o plano histórico; o código e os testes são a fonte do estado atual.
...
- [x] T01 — `LOG_MAX_LINES` em `js/balance.js`
...
- [x] T02 — Helpers compartilhados: constante, seletores e medidores
...
- [x] T03 — Casos `TECLA` e `ABERTURA` em `tests/browser.mjs`, mais a passagem de mouse
...
- [x] T04 — Casos `CHAT` em `tests/browser.mjs` com toque real
      Arquivos: `tests/browser.mjs`
...
- [x] T05 — Casos `MOCHILA` e `LOG` em `tests/browser.mjs`
      Arquivos: `tests/browser.mjs`
...
- [x] T06 — Não regressão `CHAT` do `#actionBar` em sala real
...
- [x] T07 — Dicas de tecla invisíveis no toque
      Arquivos: `styles.css`
...
- [x] T08 — `#btnChat` na célula livre ao lado do `#btnBag`
      Arquivos: `index.html`
...
- [x] T09 — Caixa e glifo do `#btnChat`, sem mexer no mouse
      Arquivos: `styles.css`
...
- [x] T10 — Ligação do alvo de chat com `openChat()`
      Arquivos: `js/main.js`
...
- [x] T11 — Mensagem de abertura por modo de entrada
      Arquivos: `js/main.js`
      Mudança: em `enterGame()` (`js/main.js:255-272`), trocar a linha fixa de `js/main.js:270` por ramificação em `matchMedia('(pointer: coarse)').matches`, com as duas strings lado a lado para a diferença ficar óbvia na revisão. As duas saem pelo mesmo `UI.pushLog(<texto>, 'system')` (RF-02 AC 4), respeitando o teto de `CHAT_LOG_LINES` (`js/ui.js:97`). Usar exatamente as strings congeladas no PLAN: a de toque é `Arraste o polegar na metade esquerda para o joystick; use os botões à direita para magias, poções e mochila; o balão ao lado da mochila abre a conversa.` (corrigida após medição em aparelho real — ver a nota na tabela de valores congelados do PLAN: citar o `aria-label` de um botão só de ícone fazia o jogador tocar no texto do `#log` e andar em vez de abrir a conversa) e a de mouse continua byte a byte igual a `Use <b>1–4</b> para magias, <b>Q/E</b> para poções, <b>Enter</b> para conversar.` com travessão U+2013. Não montar a string lendo o `aria-label` do DOM nem citá-lo no texto: RF-02 AC 5 foi invertida e agora reprova instrução que mande procurar rótulo existente só em `aria-label`.
      Cobre: RF-02, RNF-04, RNF-06
      Acceptance criteria: em 390x844 e 360x640 o `innerText` do `#log` logo após entrar não casa `/1\s*[–-]\s*4/`, `/Q\s*\/\s*E/`, `/\bTab\b/` nem `/\bEnter\b/`, casa `/joystick/i`, casa `/convers|falar|balão/i`, **não** contém o `aria-label` de nenhum controle do `#actionBar` sem texto visível (AC 5 invertida após a medição em aparelho real), e o `<p>` da linha tem a classe `system`; em 1280x760 a string é exatamente a de `js/main.js:270`; as duas strings medidas na mesma execução são diferentes entre si.
...
- [x] T12 — Alvo de fechar, `maxLength` e o par abrir/fechar no mesmo estado
      Arquivos: `js/main.js`
...
- [x] T13 — Linha de chat no toque: 44px, área segura e `#log` acima do campo
      Arquivos: `styles.css`
...
- [x] T14 — `js/ui.js` publica o teto de linhas como custom property
      Arquivos: `js/ui.js`
...
- [x] T15 — `#log` de toque em `LOG_MAX_LINES` linhas
      Arquivos: `styles.css`
...
- [x] T16 — Mochila em telas estreitas: `.equip-col` em duas colunas
      Arquivos: `styles.css`
...
- [x] T17 — Reescrever as três specs de design que esta feature derruba
...
- [x] T18 — Fechamento: regressão completa e greps de fonte única
