...
Uma observação de contexto que não vira requisito: `hud-grupo-mobile.md` §5 declara _landscape_
...
- **Out**:
  ...
  - Orientação _landscape_: os alvos confirmados são retrato; nenhum requisito abaixo mede paisagem.
...
- **UI-02** [Ubiquitous]: Com o chat aberto em `pointer: coarse`, o `#chatInput` SHALL ser operável
  ...
  - AC 2: `window.__M.intersects(rect('#chatInput'), rect('#log')) === false` com o chat aberto e
    ...
    **A AC só tem sentido quando `rect('#log')` é não-nulo**: abaixo de 460px de altura o `#log`
    ...
    operando nulo (`tests/mobile-helpers.mjs:88-92`) — ela passaria vazia justamente em 440, 380 e 360. O caso DEVE asserir `rect('#log') !== null` como pré-condição e **pular** a comparação, com
