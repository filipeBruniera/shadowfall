...
const idle = G => {
  for (const id of Object.keys(G.players)) setInput(G, id, { mx: 0, my: 0, acts: [] });
};
...
console.log('\n== cura em grupo ==');
{
  ...
  ps.forEach((p, i) => {
    p.x = G.map.spawn.x + i * 0.4;
    p.hp = stats(p).maxHp * (1 - i * 0.08);
  });
  ...
  const antes = alvo.hp;
  setInput(G, curador.id, {
    mx: 0,
    my: 0,
    acts: [{ id: 1, k: 'cast', slot: 'E', ax: curador.x, ay: curador.y }],
  });
  step(G, TICK);
  check(
    'combate em grupo: cura escolhe o aliado mais ferido entre 9',
    alvo.hp > antes,
    `hp ${antes.toFixed(1)} -> ${alvo.hp.toFixed(1)}`
  );
  ...
  check(
    'combate em grupo: a cura vai para um alvo só, não para todos',
...
}
{
  ...
  setInput(G, ps[0].id, {
    mx: 0,
    my: 0,
    acts: [{ id: 1, k: 'cast', slot: 'E', ax: ps[0].x, ay: ps[0].y }],
  });
  step(G, TICK);
  check(
    'combate em grupo: aliado fora do alcance não é curado',
...
console.log('\n== provocação em grupo ==');
{
  ...
  perto.forEach((m, i) => {
    m.x = tank.x + Math.cos(i) * 2;
    m.y = tank.y + Math.sin(i) * 2;
    m.aggro = 'p' + ((i % 9) + 1);
  });
  ...
  setInput(G, tank.id, {
    mx: 0,
    my: 0,
    acts: [{ id: 1, k: 'cast', slot: 'E', ax: tank.x, ay: tank.y }],
  });
  step(G, TICK);
  const presos = perto.filter(m => m.hp > 0 && m.tauntedBy === tank.id);
  check(
    'combate em grupo: provocação funciona com 10 jogadores no raio',
    presos.length > 0,
    `${presos.length} de ${perto.length}`
  );
  check(
    'combate em grupo: provocação muda o alvo dos monstros no raio',
...
console.log('\n== XP compartilhado ==');
{
  ...
  check(
    'XP: creditado a todos os vivos no raio, sem depender de autoria',
    ps[1].xp > antes[1] && ps[2].xp > antes[2],
    `p1 ${antes[1]}->${ps[1].xp}`
  );
  check(
    'XP: quem está além do raio não recebe',
    longe.xp === antes[3],
    `${antes[3]} -> ${longe.xp}`
  );
}
{
  check(
    'XP: divisor por raiz do número de vivos confere com 1, 2 e 10',
    Math.abs(xpShare(1) - 1) < 1e-9 &&
      Math.abs(xpShare(2) - 0.7071) < 1e-3 &&
      Math.abs(xpShare(10) - 0.3162) < 1e-3
  );
  check(
    'XP: mais gente viva significa parcela menor',
    xpShare(2) > xpShare(5) && xpShare(5) > xpShare(10)
  );
}
...
{
  check(
    'escala: a curva é monotônica',
    groupScale(1) < groupScale(4) && groupScale(4) < groupScale(10)
  );
  ...
  check(
    'escala: entrada inválida cai para 1 jogador',
    groupScale(0) === 1 && groupScale(-3) === 1 && groupScale(NaN) === 1
  );
  check(
    'escala: 10 jogadores enfrentam mais monstros que 1',
    floorPopulation(5, 10) > floorPopulation(5, 1),
    `${floorPopulation(5, 1)} -> ${floorPopulation(5, 10)}`
  );
  check(
    'escala: com 1 jogador o andar é idêntico ao balanceamento atual',
...

{
  ...
  ps.forEach((p, i) => {
    if (i > 0 && i < 9) {
      p.x = G.map.spawn.x + 40;
      p.y = G.map.spawn.y + 40;
    }
  });
  ps[0].x = caido.x + 0.5;
  ps[0].y = caido.y;
  ...
  check(
    'ressurreição: ainda não ergueu antes do tempo',
    caido.dead,
    `prog ${caido.reviveProg.toFixed(2)}`
  );
  run(G, 1);
  check('ressurreição: qualquer um dos 9 aliados no raio ergue', !caido.dead);
  check(
    'ressurreição: leva 3,5s e devolve o jogador em pé no local',
...
}
{
  ...
  caido.deathTimer = 0;
  caido.reviveProg = 0;
  ps.slice(0, 9).forEach((p, i) => {
    p.x = caido.x + Math.cos(i) * 0.6;
    p.y = caido.y + Math.sin(i) * 0.6;
  });

  run(G, REVIVE_TIME / 9);
  check(
    'ressurreição: o ganho por aliado tem teto',
    caido.dead,
    `prog ${caido.reviveProg.toFixed(2)}`
  );
  run(G, REVIVE_TIME);
...
}
{
  ...
  check(
    'ressurreição: sair do raio decai em vez de zerar',
    apos < noMeio && apos > 0,
    `${noMeio.toFixed(2)} -> ${apos.toFixed(2)}`
  );

  ps[0].x = caido.x + 0.5;
  run(G, 0.2);
  check(
    'ressurreição: voltar ao raio retoma de onde parou',
    caido.reviveProg > apos,
    `${apos.toFixed(2)} -> ${caido.reviveProg.toFixed(2)}`
  );
}
{
...
console.log('\n== portal coletivo ==');
function noPortal(G, p) {
  p.x = G.map.portal.x + 0.5;
  p.y = G.map.portal.y + 0.5;
}
function forapdoPortal(G, p) {
  p.x = G.map.portal.x + 20;
  p.y = G.map.portal.y + 20;
}

{
  ...
  run(G, PORTAL_HOLD + 1);
  check(
    'portal: com 3 vivos, 2 em cima não desce',
    G.floor === 1 && !G.pendingFloor,
    `${G.portalReady}/${G.portalTotal}`
  );
  check(
    'portal: a contagem mostra quantos vivos estão em cima',
    G.portalReady === 2 && G.portalTotal === 3,
    `${G.portalReady}/${G.portalTotal}`
  );
...
}
{
  ...
  run(G, 0.2);
  check(
    'portal: sair zera o hold coletivo',
    meio > 0 && G.portalHold === 0,
    `${meio.toFixed(2)} -> ${G.portalHold}`
  );
}
{
...
console.log('\n== composição da dificuldade do chefe ==');
{
  ...
  for (const { floor, players } of cenarios) {
    ...
    const atkEsperado = Math.floor(tipo.atk * curva.atkMult * (hc ? HARDCORE_ATK_MULT : 1));
  ...
  check(
    'chefe: maxHp e atk são o produto de curva x groupScale x fator HARDCORE',
    produtoOk,
    produtoDet
  );
  ...
  check(
    'chefe: o produto independe da ordem dos fatores',
    Math.abs(ordemA - ordemB) < 1e-9,
    `${ordemA} vs ${ordemB}`
  );
  ...
  for (const players of [1, 10]) {
    ...
    check(
      `chefe: HARDCORE tem HP e dano estritamente maiores (grupo de ${players})`,
  ...
  for (const floor of [3, 5]) {
    ...
    check(
      `chefe: HP cresce com o grupo no andar ${floor}`,
      cheio.maxHp > solo.maxHp,
      `${solo.maxHp} -> ${cheio.maxHp}`
    );
  }
  ...
  for (let f = 2; f <= 30; f++) {
    ...
    if (cur.level < prev.level || cur.hpMult < prev.hpMult || cur.atkMult < prev.atkMult)
      mono = false;
  }
...
console.log('\n== valor da curva de chefe ==');
{
  ...
  for (const floor of [1, 10, 30]) {
    ...
    check(
      `chefe: bossCurve(${floor}) devolve nível, hpMult e atkMult da curva declarada`,
      curva.level === nivel &&
  ...
  check(
    'chefe: a curva cresce de verdade entre o primeiro e o último andar medido',
...
console.log('\n== duração de luta: HARDCORE contra o comum (RF-11) ==');
{
  ...
  function medirLuta(floor, hardcore, seed) {
    ...
    for (let i = 0; i < GRUPO_MEDICAO; i++) {
      const p = addPlayer(G, {
        id: 'medic' + i,
        name: 'M' + i,
        voc: VOC_LIST[i % VOC_LIST.length],
      });
    ...
    for (; t < limite && chefe.hp > 0; t++) {
      ps.forEach((p, i) => {
        ...
        const st = stats(p);
        if (p.hp < st.maxHp * 0.65 && t % 10 === 0)
          acts.push({ id: t * 8 + 2, k: 'pot', slot: 'hp' });
        if (t % 12 === 0) {
          acts.push({
            id: t * 8 + 3,
            k: 'cast',
            slot: ['Q', 'W', 'E', 'R'][(t / 12 + i) % 4],
            ax: chefe.x,
            ay: chefe.y,
          });
        }
      ...
      ps.forEach((p, i) => {
        if (!p.dead && p.foraDePosto) {
          posto(p, i);
          p.foraDePosto = false;
        }
      });
      ps.forEach(p => {
        if (p.dead) p.foraDePosto = true;
      });
    }
    return {
      s: t * TICK,
  ...
  for (const m of medidas) {
    ...
    console.log(
      `      andar ${m.floor} ${tipo.id}: HARDCORE ${m.tHc.toFixed(1)}s (${m.mortesHc} mortes) x comum ${m.tComum.toFixed(1)}s (${m.mortesComum} mortes) → razão ${m.razao.toFixed(3)}`
    );
    check(
      `RF-11: o chefe ${tipo.id} cai nas duas variantes dentro do teto de medição`,
      m.todasCairam,
      `${m.tHc.toFixed(1)}s / ${m.tComum.toFixed(1)}s`
    );
    check(
      `RF-11: derrubar o ${tipo.id} HARDCORE custa entre 1,8x e 2,4x o tempo do comum`,
      m.razao >= RAZAO_MIN && m.razao <= RAZAO_MAX,
      `razão ${m.razao.toFixed(3)}`
    );
  }

  const iguais = medidas.every(
    (m, i) => m.tHc === repeticao[i].tHc && m.tComum === repeticao[i].tComum
  );
  check(
    'RF-11: duas execuções seguidas devolvem exatamente a mesma razão',
    iguais,
    medidas.map((m, i) => `${m.razao.toFixed(3)} vs ${repeticao[i].razao.toFixed(3)}`).join(' | ')
  );
  check(
    'RF-11: a medição devolve Math.random intacto para as suítes seguintes',
    Math.random === original
  );
}
...
console.log('\n== escala do andar ==');
{
  ...
  check(
    'escala: 10 jogadores enfrentam mais monstros e mais HP que 1',
  ...
  check(
    'escala: HP do chefe acompanha o tamanho do grupo',
  ...
  nextFloor(G);
  check(
    'escala: o andar seguinte já considera o grupo maior',
    G.groupSize === Object.keys(G.players).length
  );
}
...
}
{
  ...
  check(
    'coleta em grupo: item é coletado uma única vez com 10 jogadores no raio',
    ganharam.length === 1 && G.items.length === 0,
    `${ganharam.length} ganharam`
  );
  check(
    'coleta em grupo: nenhum item some sem entrar em mochila alguma',
...
}
{
  ...
  G.items.length = 0;
  ps[0].x = G.map.spawn.x;
  ps[0].y = G.map.spawn.y;
  ps[1].x = G.map.spawn.x + 0.7;
  ps[1].y = G.map.spawn.y;
  ps[2].x = G.map.spawn.x + 30;
  ps[2].y = G.map.spawn.y;
