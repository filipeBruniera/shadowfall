...
console.log('\n== tamanho da mensagem ==');
{
  ...
  check(
    'chat: mensagem acima do limite é truncada antes do envio',
    g.clean(longa).length === CHAT_MAX_LEN,
    `${g.clean(longa).length}`
  );
...
console.log('\n== antiflood ==');
{
  ...
  check(
    'chat: rajada acima do limite é barrada',
    ok.filter(Boolean).length === CHAT_BURST,
    `${ok.filter(Boolean).length} passaram`
  );
...
console.log('\n== log ==');
{
  ...
  check(
    'chat: o teto de linhas do log está declarado',
    CHAT_LOG_LINES >= 20 && CHAT_LOG_LINES <= 200
  );
  ...
  check(
    'chat: as linhas mais novas ficam, as antigas caem',
