...
console.log('\n== seleção do trilho ==');
{
  ...
  check('HUD: com 9 aliados apenas 3 têm barra', r.shown.length === HUD_ALLY_LIMIT, `${r.shown.length}`);
...
}
{
  ...
  for (const n of [0, 1, 3, 9]) {
    ...
    if (r.shown.length !== esperado) { check(`HUD: layout com ${n} aliados`, false, `${r.shown.length}`); break; }
  }
...
console.log('\n== estabilidade ==');
{
  ...
  check('HUD: a seleção dos 3 mais próximos é estável na fronteira', trocas === 0, `${trocas} trocas`);

  // Vantagem clara acima da histerese: aí sim troca.
  for (let i = 0; i < 40; i++) rail.select(me, [...base, ally('d', 5 - HUD_ALLY_HYSTERESIS - 1)], 1 / 30);
  check('HUD: vantagem acima da histerese troca quem aparece', rail.current.includes('d'), rail.current.join(','));
}
...
console.log('\n== aliado caído ==');
{
  ...
  const r = rail.select(me, lista, 1);
  check('HUD: aliado caído aparece mesmo fora dos 3',
  ...
  const muitos = [ally('a', 1), ...Array.from({ length: 4 }, (_, i) => ally('d' + i, 10 + i, true))];
