...
import {
  MAX_PLAYERS, PORTAL_HOLD, REVIVE_TIME, groupScale, bossCurve, HARDCORE_EVERY,
} from '../js/balance.js';
...
const semear = (seed) => { Math.random = mulberry32(seed); };
...
console.log('\n== grupo de 10 em andar HARDCORE ==');
{
  ...
  check('andar HARDCORE: o chefe do andar 9 nasce na variante HARDCORE',
  ...
  console.log(`      andar ${ANDAR_HC} (HARDCORE): portal em ${(t * TICK).toFixed(1)}s de simulação`);
  check('andar HARDCORE: o chefe cai e o portal abre dentro da janela medida',
    G.portalOpen, `${(t * TICK).toFixed(0)}s de ${(limiteHc * TICK).toFixed(0)}s`);
  check('andar HARDCORE: o chefe HARDCORE realmente morreu, não sumiu da lista',
    chefe.hp <= 0, `${chefe.hp}/${chefe.maxHp}`);
}
...
console.log('\n== orçamento de tick: 1 contra 10 ==');
{
  ...
  console.log(`      ${MAX_PLAYERS} jogadores em andar HARDCORE: ${dezHc.monstros} monstros, ${dezHc.ms.toFixed(3)}ms/tick`);
  ...
  check('grupo de 10: tick permanece dentro do orçamento também em andar HARDCORE',
    dezHc.ms < 4, `${dezHc.ms.toFixed(3)}ms no andar 9`);
...
check('grupo de 10: a medição semeou Math.random e devolveu o original',
